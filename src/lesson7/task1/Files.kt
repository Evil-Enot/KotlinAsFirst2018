...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    ...
    val newFile = File(inputName).readText().toLowerCase()
    for (i in substrings) {
        val el = i.toLowerCase()
        val count = Regex(el).findAll(newFile).toList().size
        map[i] = count
    }
    return map
}
...
fun centerFile(inputName: String, outputName: String) {
    ...
    for (i in newFile) {
        ...
        if (length == maxLength.trim().length)
            endFile.write(i)
        else {
            val dist = (maxLength.trim().length - length) / 2
...
fun alignFileByWidth(inputName: String, outputName: String) {
    ...
    for (line in newFile) {
        val words = line.split(" ").filter { it != "" }.toMutableList()
        if (words.size <= 1)
            endFile.write(line.trim())
        else {
            while (max > words.joinToString("").length)
                for (i in 0 until words.size - 1)
                    if (max <= words.joinToString("").length)
                        break
                    else
                        words[i] += " "
            endFile.write(words.joinToString(""))
        }
        endFile.newLine()
    }
    endFile.close()
